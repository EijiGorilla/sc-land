...
}`);if(e.pointDistanceEnabled&&(i.uniforms.add(new b.m("maxPixelDistance",((e,t)=>2*t.camera.computeScreenPixelSizeAt(e.pointDistanceTarget)))),i.code.add(D.H`float sphereDistancePixels(vec4 sphere, vec3 pos) {
float dist = distance(sphere.xyz, pos) - sphere.w;
float width = fwidth(dist);
dist /= min(width, maxPixelDistance);
return abs(dist);
}`)),e.intersectsLineEnabled&&i.uniforms.add(new P.U("perScreenPixelRatio",(e=>e.camera.perScreenPixelRatio))).code.add(D.H`float lineDistancePixels(vec3 start, vec3 dir, float radius, vec3 pos) {
...
//# sourceMappingURL=1307.0f451669.chunk.js.map